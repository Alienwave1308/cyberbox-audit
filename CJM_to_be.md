...
| ❌ As is | ✅ To be |
|---|---|
| Нет TG Mini App — нет канального трафика | TG Mini App: пост → кнопка → авто-авторизация |
| Нет CAPTCHA → брутфорс, фарминг | Cloudflare Turnstile на входе |
| Нет онбординга → растерянность | 3-шаговый онбординг + кнопка «Пропустить» |
| Cookies без httpOnly → XSS-кража | httpOnly + secure + sameSite cookies |
| Нет Provably Fair → нет доверия | Provably Fair + страница верификации |
| Вывод: нет статусов, нет SLA | Статус транзакции + SLA 1–24 ч + комиссии |
| Рефералы: 12% от депозита → убыток | Рефералы: % от GGR |
| Нет KYC при выводе | KYC при первом выводе |
| Только клиентская валидация адреса | Серверная валидация адреса кошелька |
| Вейджер 3 дня → давление | Вейджер 7 дней + прогресс-бар |
